...
static void inertial_scroll_work_cb(struct k_work *work) {
    ...
    for (size_t i = 0; i < cfg->codes_len && i < 2; i++) {
        ...
        if (ABS(v) < cfg->min_velocity_q8 || data->ticks > cfg->max_ticks) {
            data->velocity[i] = 0;
            continue;
        }
        ...
        data->velocity[i] = (v * cfg->decay_percent) / 100;
        if (ABS(data->velocity[i]) >= cfg->min_velocity_q8) {
            keep_running = true;
        }
    }
    ...
    if (keep_running && data->ticks <= cfg->max_ticks) {
        k_work_schedule(&data->work, K_MSEC(cfg->tick_ms));
    }
}
...
static int inertial_scroll_handle_event(const struct device *dev, struct input_event *event,
    ...
    if (idx < 0 || event->value == 0) {
        return ZMK_INPUT_PROC_CONTINUE;
    }

    data->input_dev = event->dev;
    data->velocity[idx] = ((int32_t)event->value * (int32_t)cfg->impulse_percent * Q_ONE) / 100;
    data->ticks = 0;

    /* Any new real movement restarts inertia from the latest velocity. */
    k_work_reschedule(&data->work, K_MSEC(cfg->idle_ms));
